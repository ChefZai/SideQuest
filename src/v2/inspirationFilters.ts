...
export const INSPIRATION_FILTERS=[
  {id:"for-you",label:"For You"},{id:"date-night",label:"Date Night"},{id:"food-drinks",label:"Food & Drinks"},{id:"weekend",label:"Weekend"},{id:"outdoors",label:"Outdoors"},{id:"at-home",label:"At Home"},{id:"budget-friendly",label:"Budget Friendly"},{id:"travel",label:"Travel"},{id:"something-new",label:"Something New"},{id:"games",label:"Games"},{id:"entertainment",label:"Entertainment"},{id:"learning",label:"Learning"},{id:"seasonal",label:"Seasonal"},{id:"bucket-list",label:"Bucket List"},
] as const;
export type InspirationFilterId=typeof INSPIRATION_FILTERS[number]["id"];

const tagMap:Readonly<Partial<Record<InspirationFilterId,readonly string[]>>>={"date-night":["date-night"],"food-drinks":["food-drinks"],weekend:["weekend-adventure"],outdoors:["outdoors"],"at-home":["at-home"],"budget-friendly":["budget-friendly"],"bucket-list":["bucket-list"],"something-new":["popular","bucket-list"],games:["at-home","rainy-day"],entertainment:["date-night","weekend-adventure"],learning:["bucket-list","rainy-day"]};
export function inspirationItems(items:readonly InspirationSuggestion[],filter:InspirationFilterId,spaceTemplateId:SpaceTemplateId|undefined):InspirationSuggestion[]{
  ...
  if(filter==="games")return items.filter(item=>item.templateId==="game"||tagMap.games?.some(tag=>item.tags.includes(tag)));
  if(filter==="entertainment")return items.filter(item=>["movie","event"].includes(item.templateId)||tagMap.entertainment?.some(tag=>item.tags.includes(tag)));
  if(filter==="learning")return items.filter(item=>item.templateId==="book"||item.id.includes("learn")||tagMap.learning?.some(tag=>item.tags.includes(tag)));
