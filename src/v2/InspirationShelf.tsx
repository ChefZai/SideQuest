...
export function InspirationShelf({space,profileId:_profileId,onSelect}:{space:Space;profileId:string;onSelect:(suggestion:InspirationSuggestion)=>void}){
  ...
  const wheelChips=(event:React.WheelEvent<HTMLDivElement>)=>{const row=chips.current;if(!row||row.scrollWidth<=row.clientWidth||Math.abs(event.deltaX)>=Math.abs(event.deltaY))return;const before=row.scrollLeft;row.scrollLeft+=event.deltaY;if(row.scrollLeft!==before)event.preventDefault()};
