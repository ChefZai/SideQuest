...
test("homepage controls do not cancel ordinary vertical wheel scrolling",async()=>{const source=await readFile("src/v2/InspirationShelf.tsx","utf8"),launcher=await readFile("src/v2/InspirationLauncher.tsx","utf8");assert.doesNotMatch(source,/onWheel=|wheelChips|scrollBy/);assert.doesNotMatch(launcher,/onWheel=/);assert.match(launcher,/previousContentOverflow/);assert.match(launcher,/content\.style\.overflow=previousContentOverflow/)});
