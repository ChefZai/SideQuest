...
const photoSrc = (photo: string, w: number, h: number) => {
  if (!photo) return "";
  if (
...
const DEFAULT_CATEGORIES: CategoryDef[] = [
  { id: "food",      emoji: "🍜", label: "Food",      accent: "230,118,38" },
  { id: "travel",    emoji: "✈️", label: "Travel",    accent: "40,110,220" },
  { id: "concert",   emoji: "🎵", label: "Concert",   accent: "148,68,220" },
  { id: "adventure", emoji: "🏕️", label: "Adventure", accent: "60,148,90" },
  { id: "gift",      emoji: "🎁", label: "Gift",      accent: "210,148,40" },
  { id: "together",  emoji: "❤️", label: "Together",  accent: "190,60,80" },
];

const DEFAULT_SPACE_EMOJIS = ["❤️", "🧭", "✨", "🌍", "🎯", "🔥", "🏔️", "🎪"];

const DEFAULT_REACTION_DEFS: ReactionDef[] = [
  { type: "love",  emoji: "❤️", label: "Love" },
  { type: "down",  emoji: "👍", label: "Interested" },
  ...
  return (
    <motion.div whileTap={{ scale: 0.98 }} onClick={onPress}
      ...
      <div style={{ position: "relative", height: 180 }}>
        <img src={photoSrc(idea.photo, 800, 600)} alt={idea.title}
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: BG }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "80px 1fr", gap: 10, marginBottom: 24 }}>
          <div style={{ background: CARD, borderRadius: 14, border: `1px solid ${BORDER}`,
            padding: "14px 12px" }}>
            <div style={{ fontFamily: SANS, fontSize: 11, color: MUTED, fontWeight: 600,
              letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 6 }}>Emoji</div>
            <input value={emoji} onChange={e => setEmoji(e.target.value)}
              placeholder="✨" aria-label="Space emoji"
              style={{ fontFamily: SANS, fontSize: 22, color: FG, textAlign: "center" }} />
          </div>
          <div style={{ background: CARD, borderRadius: 14, border: `1px solid ${BORDER}`,
            padding: "14px 16px" }}>
            <div style={{ fontFamily: SANS, fontSize: 11, color: MUTED, fontWeight: 600,
              ...
              {coverIdeas.length >= 4 ? (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", height: 130 }}>
                  {coverIdeas.map((idea, i) => (
                    <img key={i} src={photoSrc(idea.photo, 200, 130)} alt=""
...
const [selectedPhoto, setSelectedPhoto] = useState<string | null>(
  ...
  const availableCategories = [
    ...(targetSpace?.categories?.length ? targetSpace.categories : DEFAULT_CATEGORIES),
    ...customCategories,
  ];
  ...
  const handleSave = () => {
    ...
    setError("");
    const cat = availableCategories.find(c => c.label === selectedCat);
      ...
      <div style={{ padding: "20px 20px 40px", display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
          {availableCategories.map(cat => {
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                onClick={() => {
                  setSelectedCat(isSel ? "" : cat.label);
                  setError("");
                }}
                style={{
                  flexShrink: 0,
                  border: `1.5px solid ${isSel ? `rgb(${cat.accent})` : BORDER}`,
                  ...
                  fontFamily: SANS,
                  fontSize: 13,
                  color: isSel ? `rgb(${cat.accent})` : MUTED,
          ...
          <motion.button
            ...
            style={{
              flexShrink: 0,
              border: `1.5px solid ${showCustomCategory ? TEAL : BORDER}`,
              background: showCustomCategory ? "rgba(61,155,137,0.14)" : CARD,
              borderRadius: 20,
              padding: "7px 14px",
              cursor: "pointer",
              display: "flex",
              ...
              fontFamily: SANS,
              fontSize: 13,
              color: showCustomCategory ? TEAL : MUTED,
              fontWeight: 600,
            }}
          >
            <Plus size={12} /> Custom
          ...
          >
            ...
            <motion.button
              ...
              style={{
                border: "none",
                borderRadius: 10,
                background: customCategoryLabel.trim() ? TEAL : "rgba(61,155,137,0.25)",
                color: FG,
                fontFamily: SANS,
                fontSize: 13,
                fontWeight: 700,
                padding: "10px 12px",
                cursor: customCategoryLabel.trim() ? "pointer" : "not-allowed",
              }}
            >
              Add
            </motion.button>
...
function IdeaDetailOverlay({ ideaId, ideas, spaces, currentUser, onClose, onMarkDone, onDelete, onEdit, onUpdateReaction, onAddComment }: {
  ...
  const reactionDefs = space?.reactionDefs?.length ? space.reactionDefs : DEFAULT_REACTION_DEFS;
  ...
  const reactionCounts = reactionDefs.map(r => ({
    ...r,
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {Object.entries(idea.reactions).map(([member, reaction]) => {
                const def = reactionDefs.find(r => r.type === reaction);
                return (
...
function CreateSpaceOverlay({ spaces, onSave, onClose }: {
  ...
  const [reactionDefs, setReactionDefs] = useState<ReactionDef[]>(
    DEFAULT_REACTION_DEFS.map(reaction => ({ ...reaction }))
  );
  const [selected, setSelected] = useState<string[]>([]);
  ...
  const addCategory = () => {
    ...
    setCategories(prev => [
      ...prev,
      { id, emoji: "✨", label: "Custom", accent: "61,155,137" },
    ]);
  };
  ...
  const updateReactionDef = (type: string, patch: Partial<ReactionDef>) => {
    setReactionDefs(prev => prev.map(reaction =>
      reaction.type === type ? { ...reaction, ...patch } : reaction
    ));
  };

  const addReactionDef = () => {
    const type = `custom-${Date.now()}`;
    setReactionDefs(prev => [
      ...prev,
      { type, emoji: "✨", label: "Custom" },
    ]);
  };

  const removeReactionDef = (type: string) => {
    setReactionDefs(prev => prev.filter(reaction => reaction.type !== type));
  };
        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ background: CARD, borderRadius: 14, border: `1px solid ${BORDER}`,
              ...
              <span style={{ fontFamily: SANS, fontSize: 12, color: MUTED, whiteSpace: "nowrap" }}>Custom emoji</span>
              <input
                value={emoji}
                onChange={e => setEmoji(e.target.value)}
                placeholder="✨"
                aria-label="Custom space emoji"
                style={{ fontFamily: SANS, fontSize: 24, color: FG, textAlign: "center" }}
              />
            </div>
            {/* Name input */}
            <div style={{ background: CARD, borderRadius: 14, border: `1px solid ${BORDER}`,
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {categories.map(cat => (
                  <div key={cat.id} style={{ display: "grid", gridTemplateColumns: "64px 1fr auto",
                    gap: 8, alignItems: "center", background: CARD, border: `1px solid ${BORDER}`,
                    borderRadius: 12, padding: 8 }}>
                    <input
                      value={cat.emoji}
                      onChange={e => updateCategory(cat.id, { emoji: e.target.value })}
                      aria-label={`${cat.label} emoji`}
                      style={{ fontFamily: SANS, fontSize: 22, textAlign: "center",
                        background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "6px" }}
                    />
                    <input
                      value={cat.label}
                      onChange={e => updateCategory(cat.id, { label: e.target.value })}
                      aria-label="Category label"
                      style={{ fontFamily: SANS, fontSize: 14, color: FG,
                        background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "8px 10px" }}
                    />
                    <motion.button whileTap={{ scale: 0.9 }} onClick={() => removeCategory(cat.id)}
                      disabled={categories.length <= 1}
                      style={{ width: 34, height: 34, borderRadius: 9, border: `1px solid ${BORDER}`,
                        background: "rgba(255,255,255,0.03)", cursor: categories.length <= 1 ? "not-allowed" : "pointer",
                        display: "flex", alignItems: "center", justifyContent: "center",
                        opacity: categories.length <= 1 ? 0.35 : 1 }}>
                      <X size={14} color={MUTED} />
                    </motion.button>
                  </div>
                ))}
              </div>
              <motion.button whileTap={{ scale: 0.97 }} onClick={addCategory}
                style={{ marginTop: 10, background: "none", border: `1px dashed ${BORDER}`,
                  color: TEAL, borderRadius: 12, padding: "9px 12px", width: "100%",
            ...
            <div style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {reactionDefs.map(reaction => (
                  <div key={reaction.type} style={{ display: "grid", gridTemplateColumns: "64px 1fr auto",
                    gap: 8, alignItems: "center", background: CARD, border: `1px solid ${BORDER}`,
                    borderRadius: 12, padding: 8 }}>
                    <input
                      value={reaction.emoji}
                      onChange={e => updateReactionDef(reaction.type, { emoji: e.target.value })}
                      aria-label={`${reaction.label} emoji`}
                      style={{ fontFamily: SANS, fontSize: 22, textAlign: "center",
                        background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "6px" }}
                    />
                    <input
                      value={reaction.label}
                      onChange={e => updateReactionDef(reaction.type, { label: e.target.value })}
                      aria-label="Reaction label"
                      style={{ fontFamily: SANS, fontSize: 14, color: FG,
                        background: "rgba(255,255,255,0.04)", borderRadius: 8, padding: "8px 10px" }}
                    />
                    <motion.button whileTap={{ scale: 0.9 }} onClick={() => removeReactionDef(reaction.type)}
                      disabled={reactionDefs.length <= 1}
                      style={{ width: 34, height: 34, borderRadius: 9, border: `1px solid ${BORDER}`,
                        background: "rgba(255,255,255,0.03)", cursor: reactionDefs.length <= 1 ? "not-allowed" : "pointer",
                        display: "flex", alignItems: "center", justifyContent: "center",
                        opacity: reactionDefs.length <= 1 ? 0.35 : 1 }}>
                      <X size={14} color={MUTED} />
                    </motion.button>
                  </div>
                ))}
              </div>
              <motion.button whileTap={{ scale: 0.97 }} onClick={addReactionDef}
                style={{ marginTop: 10, background: "none", border: `1px dashed ${BORDER}`,
                  color: TEAL, borderRadius: 12, padding: "9px 12px", width: "100%",
                  fontFamily: SANS, fontSize: 13, cursor: "pointer" }}>
                + Add custom reaction
